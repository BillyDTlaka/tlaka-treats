...
export default function CustomerHome() {
  ...
  const fetchProducts = useCallback(async () => {
    try { const data = await api.products.getAll(); setProducts(data); setFiltered(data) }
      ...
      <div style={{ background: '#8B3A3A', padding: '52px 16px 16px', flexShrink: 0 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
          <div>
            ...
            <p style={{ fontSize: 13, color: '#f5d0d0', marginTop: 2 }}>Shop as customer</p>
          </div>
              ...
              return (
                <div key={item.id} className="product-card" onClick={() => navigate(`/customer/product/${item.id}`)}>
                  ...
                  <div style={{ padding: 12 }}>
                    ...
                    {fromPrice != null ? <p style={{ fontSize: 14, fontWeight: 700, color: '#8B3A3A' }}>From R{fromPrice.toFixed(2)}</p> : <p style={{ fontSize: 12, color: '#bbb', fontStyle: 'italic' }}>Price on request</p>}
                  </div>
                </div>
              )
            })}
