    ...
    return (
      <div className="screen" style={{ background: '#FDF6F0' }}>
        ...
        <div className="empty-state" style={{ flex: 1 }}>
          ...
          <button className="btn-primary" style={{ width: 'auto', padding: '14px 24px' }} onClick={() => navigate('/ambassador/shop')}>Browse Products</button>
        </div>
        <AmbassadorNavBar />
      </div>
    )
  ...
  return (
    <div className="screen" style={{ background: '#FDF6F0' }}>
      ...
      <div style={{ padding: 16 }}>
        ...
        <button onClick={() => { setAmbSearch(''); setShowAmbPicker(true) }}
        ...
        <textarea className="form-textarea" style={{ height: 80, marginBottom: 16 }} placeholder="e.g. No nuts, extra packaging..." value={notes} onChange={e => setNotes(e.target.value)} />
      ...
      {showAmbPicker && (
        <div className="modal-overlay" onClick={() => setShowAmbPicker(false)}>
          <div className="modal-sheet" onClick={e => e.stopPropagation()}>
            <p style={{ fontSize: 17, fontWeight: 700, textAlign: 'center', marginBottom: 14 }}>Select Ambassador</p>
            <input style={{ width: '100%', background: '#f5f0eb', borderRadius: 10, padding: 12, fontSize: 15, border: 'none', outline: 'none', marginBottom: 12 }} placeholder="Search by code…" value={ambSearch} onChange={e => setAmbSearch(e.target.value.toUpperCase())} autoFocus />
            <div style={{ maxHeight: 300, overflowY: 'auto' }}>
              {filteredAmbs.length === 0 ? (
                ...
                <button key={a.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%', padding: '14px 0', background: 'none', borderBottom: '1px solid #f5f0eb', cursor: 'pointer' }}
