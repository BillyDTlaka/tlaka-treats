...
export default function AddressAutocomplete({ value, onChange, placeholder }: Props) {
  ...
  const searchAddress = useCallback(async (text: string) => {
    ...
    try {
      ...
      const res = await fetch(url, {
        headers: {
          'Accept-Language': 'en',
          'User-Agent': 'TlakaTreats-App/1.0',
        },
      })
  ...
  const buildCleanAddress = (item: NominatimResult): string => {
    ...
    // Fallback: take first 4 parts of the display name
  ...
  const showManualHint =
    !manualMode && !showDropdown && !loading && query.length >= 3
  ...
  return (
    <View style={styles.wrapper}>
      ...
      <View style={[
        ...
        manualMode && styles.inputContainerManual,
      ]}>
        ...
        <TextInput
          ...
          onFocus={() => {
            if (!manualMode && suggestions.length > 0) setShowDropdown(true)
          }}
        />
        {loading && (
          <ActivityIndicator size="small" color="#8B3A3A" style={styles.spinner} />
        )}
      </View>
      ...
      {showDropdown && (
        <View style={styles.dropdown}>
          ...
          {/* Manual entry option always visible at bottom of dropdown */}
          <TouchableOpacity
          ...
          >
            ...
            <Text style={styles.manualDropdownText}>
      ...
      {/* Hint shown when search has results but dropdown is closed */}
      {showManualHint && (
        <TouchableOpacity style={styles.hintRow} onPress={enableManualMode}>
          <Text style={styles.hintText}>
            Rural area or address not found?{' '}
            <Text style={styles.hintLink}>Enter manually</Text>
          </Text>
        </TouchableOpacity>
      )}
      ...
      {manualMode && (
        <View style={styles.manualBadge}>
          <Text style={styles.manualBadgeText}>
            Manual entry — type your address in full
          </Text>
